...
class BASE_RNN():
    ...
    def create_graph(self):
        ...
        if self.DNN_MODEL:
            ...
            # input_x = tf.reshape(tf.tile(input, [1, self.MAX_SEQ_LEN]),
            # [BATCH_SIZE, self.MAX_SEQ_LEN, self.FEATURE_SIZE * self.EMB_DIM])
            rnn_cell = None
            ...
            new_output = tf.reshape(outputs, [int(self.MAX_SEQ_LEN * BATCH_SIZE), int(self.STATE_SIZE)])
        ...
        self.preds = preds
        survival_rate = preds
        batch_rnn_survival_rate = tf.reshape(survival_rate, [int(BATCH_SIZE), int(self.MAX_SEQ_LEN)])
        ...
        def reduce_mul(x):
            bid_len = tf.cast(x[int(self.MAX_SEQ_LEN)], dtype=tf.int32)
            market_len = tf.cast(x[int(self.MAX_SEQ_LEN + 1)], dtype=tf.int32)
        ...
        for step in range(1, self.TRAING_STEPS + 1):
            ...
            if self.ONLY_TRAIN_ANLP:
                if win:  # if win
                    ...
                    train_loss, train_outputs = sess.run([self.cost, self.predict],
                                                         feed_dict={self.tf_x: batch_x,
                                                                    self.tf_y: batch_y,
                                                                    self.tf_bid_len: batch_len,
                                                                    self.tf_market_price: batch_market_price,
    ...
    def getStatStr(self, category, step, mean_auc, mean_loss, mean_anlp):
        statistics_log = str(self.INPUT_FILE) + "\t" + \
                         category + "\t" + \
                         str(step) + "\t" "{:.6f}".format(mean_loss) + "\t" + \
                         "{:.4f}".format(mean_auc) + "\t" + \
                         "{:.4f}".format(mean_anlp) + "\t" + \
